    ...
    <table class="table table-hover">
     ...
     <tr>
        ...
        <tbody>
          ...
          $id=$_GET['id'];
           $query="select did from user where uid=$id";
           $result=mysqli_query($con,$query);
					 while ($row=mysqli_fetch_array($result)){
								$name=$row ['did'];
$qu="select * from diet where wid=$name";
...
$wname=$row['ename'];
    ...
    $n4=$row ['e4'];
   $n5=$row ['e5'];
  $q1="select * from food where eid=$n1 or eid=$n2 or eid=$n3 or eid=$n4 or eid=$n5";
  $re=mysqli_query($con,$q1);
	echo "Diet Name: ".$wname;
